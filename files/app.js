...
const SCREENS_PEGAWAI = ['dashboard','kasir','stok','riwayat','pembelian','retur','pelanggan','opname','profil'];
    ...
    bukaModal({
      ...
      bodyHtml: `
        ...
        <div class="btn-row">
          ...
          <button class="btn btn-primary" id="modal-lanjut">Mulai Shift</button>
        </div>`,
      onMount: (root) => {
...
SCREEN_RENDERERS.dashboard = async function (root) {
  ...
  const perluShift = u.role !== 'Owner' && u.wajibGPS;
  const shiftAktif = u.shiftAktif && u.shiftAktif.status === 'Aktif';

  let gpsCardHtml = '';
  if (perluShift || u.role === 'Owner') {
    gpsCardHtml = `
      <div class="gps-status">
        ...
        <div style="flex:1;">
          ...
          <div style="font-size:11.5px;color:var(--text-dim);">${shiftAktif ? 'Mulai: ' + formatTanggalJam(u.shiftAktif.mulai) : (perluShift ? 'Harus berada di lokasi apotek untuk memulai' : 'Opsional untuk Owner')}</div>
        </div>
        ${shiftAktif
...
function totalKeranjang() {
  return AppState.cart.reduce((s, x) => s + x.qty * x.hargaSatuan, 0);
}
...
function renderCartFab() {
  ...
  if (AppState.currentScreen !== 'kasir') { if (fab) fab.remove(); return; }
  if (!AppState.cart.length) { if (fab) fab.remove(); return; }
  if (!fab) {
  ...
  async function muat() {
    ...
    listEl.innerHTML = data.map(t => `
      <div class="list-item" data-id="${t.ID_Transaksi}">
        <div class="li-main">
          <div class="li-title">${t.ID_Transaksi}</div>
          <div class="li-sub">${formatTanggalJam(t.Tanggal)} • ${escapeHtml(t.Nama_Kasir)}</div>
        </div>
        <div class="li-right">
...
SCREEN_RENDERERS.pembelian = async function (root) {
  root.innerHTML = `
    <div class="container">
      <button class="btn btn-primary" id="btn-tambah-pembelian" style="margin-bottom:12px;">+ Catat Pembelian Baru</button>
      <div id="pb-list"></div>
    </div>`;
...
async function bukaFormPembelianModal() {
  ...
  bukaModal({
    ...
    bodyHtml: `
      ...
      <div class="form-group"><label>No. Faktur (opsional)</label><input type="text" id="pb-faktur"></div>
    ...
    onMount: (root) => {
      ...
      root.querySelector('#btn-simpan-pembelian').addEventListener('click', async () => {
        ...
        try {
          ...
          await apiPost('addPembelian', withIdUser({
            ...
            noFaktur: root.querySelector('#pb-faktur').value
          }));
          toast('Pembelian dicatat, stok bertambah.', 'success');
          tutupModal();
          invalidasiCacheProduk();
          renderScreen('pembelian');
        } catch (err) { tampilkanError(err); btn.disabled = false; btn.textContent = 'Simpan Pembelian'; }
      });
    }
  });
}
...
SCREEN_RENDERERS.retur = async function (root) {
  root.innerHTML = `
    <div class="container">
      <button class="btn btn-primary" id="btn-tambah-retur" style="margin-bottom:12px;">+ Buat Retur Baru</button>
      <div id="rt-list"></div>
    </div>`;
...
SCREEN_RENDERERS.profil = async function (root) {
  ...
  root.innerHTML = `
    <div class="container">
      ...
      <button class="btn btn-outline" id="btn-ganti-password" style="margin-bottom:10px;">🔑 Ganti Password</button>
  ...
  document.getElementById('btn-ganti-password').addEventListener('click', bukaGantiPasswordModal);
