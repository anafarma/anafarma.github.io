 ...
 * V18.10
 *
 * Satu bridge untuk menjaga kontrak antar layer DEV:
 * - menyamakan bentuk cart internal dengan kontrak createTransaksi production;
 * - membuka alur Pembayaran sebelum createTransaksi;
 * - memvalidasi item sebelum dikirim agar tidak lagi mendapat
 *   "Item transaksi tidak valid";
 * - mempertahankan dukungan offline/outbox createTransaksi;
 * - menampilkan LOKASI RAK -> STOK -> HARGA pada detail keranjang;
 * - tidak membuat API, router, IndexedDB, atau outbox baru.
 *
...
(function(){
  ...
  function install(){
    ...
    window.renderKasirCartStatus=function(){ scheduleCartRender(); };
    ...
    window.prosesCheckout=function(){ return mulaiPembayaran(); };
  ...
  function getProductForCart(item){
    ...
    return list.find(p=>String(p?.Kode_Obat??p?.kode??p?.Kode??p?.idProduk??p?.IDProduk??'')===key) ||
      list.find(p=>String(p?.idProduk??p?.IDProduk??'')===key) || null;
  }

  function normalizeCartItem(item){
    const product=getProductForCart(item);
    const kode=String(item?.kodeObat||item?.kode||item?.idProduk||
      product?.Kode_Obat||product?.kode||product?.Kode||'').trim();
    ...
    const satuan=String(item?.namaSatuan||item?.satuanJual||product?.Satuan||'Pcs').trim() || 'Pcs';
    return {
      original:item,
      product,
      kodeObat:kode,
      ...
      hargaSatuan:harga,
      stokTersedia:stok,
      satuanJual:String(item?.satuanJual||'normal'),
      namaSatuan:satuan
    };
  }
      ...
      try{
        ...
        if(result && result.stok!==undefined){
          ...
          if(available<required){
            return {ok:false,error:`Stok ${item.namaObat} tidak mencukupi. Tersedia ${available}, dibutuhkan ${required}.`};
          }
        }
      }catch(error){
        // Verifikasi cepat hanya optimasi. Server createTransaksi tetap menjadi
        // pemeriksaan final sehingga kegagalan endpoint verifikasi tidak
        // memblokir transaksi yang sebenarnya valid.
        console.warn('[DEV CHECKOUT VERIFY]',error);
      }
    }
    return {ok:true};
  }
  ...
  function openPaymentModal(customers){
    ...
    const options='<option value="">-- Tanpa Pelanggan --</option>'+
      (Array.isArray(customers)?customers:[]).map(p=>`<option value="${esc(p.ID_Pelanggan??p.idPelanggan??'')}">${esc(p.Nama??p.nama??'-')} (${Number(p.Poin??p.poin??0)} poin)</option>`).join('');

    const modal=document.createElement('div');
    modal.id=MODAL_ID;
    modal.className='modal-overlay center-align';
    modal.innerHTML=`
      <div class="modal-sheet modal-center" style="max-width:460px;width:min(100%,460px);">
        <div class="modal-header">
          <div><h3>Pembayaran</h3><div style="font-size:11px;color:var(--text-dim);margin-top:2px;">${validation.items.length} jenis obat • ${money(subtotal)}</div></div>
          <button type="button" class="modal-close" data-pay-close aria-label="Tutup">×</button>
        </div>
        <div class="modal-body">
          <div class="form-group"><label>Pelanggan (opsional)</label><select id="dev-pay-customer">${options}</select></div>
          <div class="form-group"><label>Diskon (Rp)</label><input id="dev-pay-discount" type="number" min="0" value="0" inputmode="numeric"></div>
          <div class="form-group"><label>Metode Pembayaran</label><select id="dev-pay-method"><option value="Tunai">Tunai</option><option value="QRIS">QRIS</option><option value="E-Wallet">E-Wallet</option></select></div>
          <div class="form-group"><label>Jumlah Dibayar</label><input id="dev-pay-paid" type="number" min="0" value="${subtotal}" inputmode="numeric"></div>
          <div style="background:var(--bg);border-radius:12px;padding:12px;margin:10px 0 14px;font-size:13px;">
            <div style="display:flex;justify-content:space-between"><span>Subtotal</span><strong>${money(subtotal)}</strong></div>
            <div style="display:flex;justify-content:space-between;margin-top:7px"><span>Total</span><strong id="dev-pay-total">${money(subtotal)}</strong></div>
            <div style="display:flex;justify-content:space-between;margin-top:7px"><span>Kembalian</span><strong id="dev-pay-change">${money(0)}</strong></div>
          </div>
          <div id="dev-pay-status" style="font-size:12px;color:var(--text-dim);text-align:center;min-height:18px;margin-bottom:8px;"></div>
          <button type="button" class="btn btn-primary" id="dev-pay-submit">Proses &amp; Simpan Transaksi</button>
        </div>
      </div>`;
    document.body.appendChild(modal);
    requestAnimationFrame(()=>modal.classList.add('show'));

    const discount=modal.querySelector('#dev-pay-discount');
    const paid=modal.querySelector('#dev-pay-paid');
    const totalEl=modal.querySelector('#dev-pay-total');
    const changeEl=modal.querySelector('#dev-pay-change');
    const status=modal.querySelector('#dev-pay-status');
    const submit=modal.querySelector('#dev-pay-submit');
    ...
    const update=()=>{
      ...
      submit.disabled=checkoutBusy||bayar<total;
    };
    discount.addEventListener('input',update);
    paid.addEventListener('input',update);
    modal.querySelector('[data-pay-close]').onclick=closePaymentModal;
    ...
    submit.onclick=async()=>{
      ...
      const current=validateCart();
      if(!current.ok){toast(current.error,'error');closePaymentModal();return;}
      checkoutBusy=true;
      submit.disabled=true;
      submit.textContent='Memproses...';
      status.textContent=AppState.isOnline?'Memverifikasi stok...':'OFFLINE — transaksi akan disimpan di perangkat.';
      try{
        const verification=await verifyStock(current.items);
        if(!verification.ok) throw new Error(verification.error);
        status.textContent='Menyimpan transaksi...';
        const requestId=typeof uuidKecil==='function'?uuidKecil():String(Date.now());
        const payload=withIdUser({
          items:backendItems(current.items),
          idPelanggan:modal.querySelector('#dev-pay-customer').value||'',
          diskon,
          pajak:0,
          metodeBayar:modal.querySelector('#dev-pay-method').value,
          bayar
        });
        ...
        closePaymentModal();
        if(result?.queued){
          toast('Transaksi disimpan offline dan akan dikirim saat online.','warn');
        }else{
          toast('Transaksi berhasil disimpan.','success');
          showReceipt(result,total,bayar);
        }
        if(typeof window.renderCart==='function') window.renderCart();
      }catch(error){
        console.error('[DEV CHECKOUT]',error);
        toast(error?.message||String(error),'error');
        submit.disabled=false;
        submit.textContent='Proses & Simpan Transaksi';
        checkoutBusy=false;
        status.textContent='Periksa data lalu coba lagi.';
        update();
        return;
      }
      checkoutBusy=false;
  ...
  function showReceipt(result,total,bayar){
    ...
    document.body.appendChild(modal);
    requestAnimationFrame(()=>modal.classList.add('show'));
    modal.querySelectorAll('[data-close-receipt]').forEach(b=>b.onclick=()=>{modal.classList.remove('show');setTimeout(()=>modal.remove(),180);});
  }
  ...
  async function mulaiPembayaran(){
    ...
    const validation=validateCart();
    if(!validation.ok){toast(validation.error,'warn');return;}
    let customers=[];
    if(AppState.isOnline&&typeof apiGet==='function'){
      try{customers=await apiGet('getPelanggan',{idUser:AppState.user.idUser},{cache:true,maxAge:2*60*1000});}
      catch(error){console.warn('[DEV CHECKOUT CUSTOMER]',error);}
    }
  ...
  function installCheckoutInterceptor(){
    document.addEventListener('click',function(event){
      const trigger=event.target.closest?.('[data-action="checkout"]');
      if(!trigger) return;
      if(!AppState.user||AppState.currentScreen!=='kasir') return;
      event.preventDefault();
      event.stopImmediatePropagation();
      mulaiPembayaran().catch(error=>{checkoutBusy=false;console.error('[DEV PAYMENT]',error);toast(error?.message||String(error),'error');});
    },true);
  }
  ...
  function renderCartDetail(){
    ...
    if(!root) return;
    if(!Array.isArray(AppState.cart)||!AppState.cart.length){
      root.innerHTML='<div class="empty-state"><div class="empty-icon">🛒</div><div>Keranjang masih kosong.</div></div>';
      return;
    }
    root.innerHTML=AppState.cart.map(item=>{
      ...
      return `<div class="list-item" style="align-items:center">
        <div class="li-main">
          <div class="li-title">${esc(n.namaObat)}</div>
          <div class="li-sub">Rak: <strong style="color:var(--primary-dark)">${esc(rack||'Belum teridentifikasi')}</strong> · Stok ${Number(n.stokTersedia)||0} · ${money(n.hargaSatuan)}</div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
          <button class="btn btn-secondary btn-sm" data-action="cart-minus" data-id-produk="${esc(n.kodeObat)}" aria-label="Kurangi">−</button>
          <strong>${Number(n.qty)||0}</strong>
          <button class="btn btn-secondary btn-sm" data-action="cart-plus" data-id-produk="${esc(n.kodeObat)}" aria-label="Tambah">+</button>
        </div>
        <div class="li-right"><div class="li-value">${money((Number(n.hargaSatuan)||0)*(Number(n.qty)||0))}</div></div>
      </div>`;
    }).join('');
  }
  ...
  function updateDockSafe(){
    const fn=window.__ANA_FARMA_KASIR_UPDATE_DOCK__;
  ...
  function installCartRenderer(){
    const attach=()=>{
      const root=document.querySelector('[data-cart-root]');
      if(!root||root.__devCompatAttached) return;
      root.__devCompatAttached=true;
      if(cartObserver) cartObserver.disconnect();
      cartObserver=new MutationObserver(()=>{
        if(root.__devCompatRendering) return;
        scheduleCartRender();
      });
