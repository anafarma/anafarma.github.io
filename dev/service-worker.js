/**
 * APOTEK ANA FARMA — DEV SERVICE WORKER V18.2
 *
 ...
 *
 * Migration guard:
 * browser tabs that still hold an older app.js may call the previous
 * Apps Script deployment. Those requests are redirected to the new DEV
 * deployment without touching production.
 */
const CACHE_VERSION='ana-farma-dev-v18-2';
...
function normalizeApiRequest(request){
  if(request.url!==LEGACY_API_URL)return request;
  const target=new URL(NEW_API_URL);
