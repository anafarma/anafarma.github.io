...
const API_URL =
  'https://script.google.com/macros/s/AKfycbxcAPMcgyu7eNyEISUvIJHs0grSIFJCJ_DOnM2wX_b2gWfWUn9VSUi4sF81X9ndz5JU/exec';

const API_DEPLOYMENT_ID =
  'AKfycbxcAPMcgyu7eNyEISUvIJHs0grSIFJCJ_DOnM2wX_b2gWfWUn9VSUi4sF81X9ndz5JU';

const APP_VERSION = '2026-08-27-DEV-OFFLINE-V18-1';
...
async function apiPost(action, data = {}, options = {}) {
  ...
  try {
    return await requestPostOnline(
      action,
      data,
      requestId
    );
  } catch (error) {
    if (bolehOffline && isRetryableTransportError(error)) {
      await masukkanOutbox(
        action,
        data,
        requestId
...
function retryDelay(retryCount) {
  ...
  return Math.min(60000, 1000 * Math.pow(2, n));
}

async function tandaiOutbox(requestId, patch) {
  const current = await dbGet('outbox', requestId);
  if (!current) return;
...
async function prosesSatuOutbox(item) {
  ...
  await tandaiOutbox(item.requestId, {
    status: 'syncing'
  });
  ...
  try {
    ...
    // Hapus hanya setelah server sukses.
...
async function sinkronkanOutbox() {
  if (AppState.syncRunning || !AppState.isOnline) {
    return;
  }
...
function jadwalkanSync(delay = 1200) {
  clearTimeout(AppState.syncTimer);

  AppState.syncTimer = setTimeout(() => {
    sinkronkanOutbox().catch(tampilkanError);
  }, delay);
}
...
async function updateOfflineUI() {
  ...
  if (banner) {
    ...
    banner.textContent = AppState.isOnline
      ? 'ONLINE'
      : 'OFFLINE — Transaksi baru akan disimpan di perangkat.';
  }
...
function scheduleAutoLogout() {
  ...
  const minutes =
    Number(
      AppState.pengaturan.autoLogoutMenit ||
      DEFAULT_AUTO_LOGOUT_MIN
    );

  if (!AppState.user || minutes <= 0) return;
...
async function login(username, password) {
  const user = await requestPostOnline(
    'login',
    {
      username: username,
      password: password
...
function registerScreen(name, renderer) {
  SCREEN_RENDERERS[name] = renderer;
}
...
registerScreen(
  ...
  async function renderDashboard() {
      ...
      <div class="container">
        ...
        <div class="grid-2">
          <div class="stat-card">
            ...
            <div class="stat-value" data-dashboard="totalProduk">—</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Stok Menipis</div>
            <div class="stat-value" data-dashboard="stokMenipis">—</div>
          </div>
          <div class="stat-card">
            <div class="stat-label">Penjualan Hari Ini</div>
            <div class="stat-value" data-dashboard="penjualanHariIni">—</div>
          </div>
          <div class="stat-card good">
            <div class="stat-label">Transaksi Hari Ini</div>
            <div class="stat-value" data-dashboard="transaksiHariIni">—</div>
          </div>
        </div>

        <div class="section-title">Akses Cepat</div>
        <div class="menu-grid">
          <button class="menu-item menu-item-unggulan" data-action="navigate" data-screen="kasir">
            <span class="menu-icon">🛒</span>
            <span class="menu-label">Kasir</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="stok">
            <span class="menu-icon">💊</span>
            <span class="menu-label">Stok</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="riwayat">
            <span class="menu-icon">🧾</span>
            <span class="menu-label">Riwayat</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="pembelian">
            <span class="menu-icon">📦</span>
            <span class="menu-label">Pembelian</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="retur">
            <span class="menu-icon">↩️</span>
            <span class="menu-label">Retur</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="pelanggan">
            <span class="menu-icon">👥</span>
            <span class="menu-label">Pelanggan</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="laporan">
            <span class="menu-icon">📊</span>
            <span class="menu-label">Laporan</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="opname">
            <span class="menu-icon">📋</span>
            <span class="menu-label">Opname</span>
          </button>
          <button class="menu-item" data-action="navigate" data-screen="pengaturan">
            <span class="menu-icon">⚙️</span>
        ...
        <div class="card">
          <div class="list-item" style="box-shadow:none;margin:0;padding:0;">
            <div class="li-main">
              <div class="li-title">Koneksi</div>
              <div class="li-sub" data-status-online>Memeriksa…</div>
            </div>
            <span class="pill pill-gray" data-status-sync>Sync</span>
          </div>
        </div>
      </div>
    `;
    ...
    if (onlineEl) {
      onlineEl.textContent = AppState.isOnline ? 'Online' : 'Offline';
    }

    if (syncEl) {
      const outbox = await jumlahOutbox().catch(() => 0);
      syncEl.textContent = outbox > 0
        ? `${outbox} pending`
        : 'Tersinkron';
      syncEl.className = outbox > 0
        ? 'pill pill-warn'
        : 'pill pill-success';
    }
...
async function muatProdukKasir() {
  ...
  AppState.produkCache = Array.isArray(data)
    ? data.map(normalisasiProduk)
    : [];
...
function tambahKeKeranjang(product, qty = 1) {
  ...
  const existing = AppState.cart.find(
    item => String(item.idProduk) === String(p.idProduk)
  );

  const currentQty = existing ? Number(existing.qty) : 0;
  const targetQty = currentQty + jumlah;

  if (Number.isFinite(p.stok) && targetQty > p.stok) {
    toast(`Stok ${p.nama} hanya ${p.stok}.`, 'warn');
    return false;
  }
...
function ubahQtyKeranjang(idProduk, delta) {
  const item = AppState.cart.find(
    x => String(x.idProduk) === String(idProduk)
  );
  ...
  if (next <= 0) {
    ...
    if (produk && next > Number(produk.stok)) {
      toast(`Stok ${produk.nama} hanya ${produk.stok}.`, 'warn');
      return;
    }
...
function renderCart() {
  ...
  if (count) {
    count.textContent = String(jumlahItemKeranjang());
  }

  if (total) {
    total.textContent = formatRupiah(totalKeranjang());
  }
  ...
  root.innerHTML = AppState.cart.map(item => `
    <div class="list-item">
      <div class="li-main">
        <div class="li-title">${escapeHtml(item.nama)}</div>
        <div class="li-sub">${escapeHtml(item.kode)} · ${formatRupiah(item.hargaSatuan)}</div>
      </div>
      <div class="btn-row" style="align-items:center;">
        <button class="btn btn-secondary btn-sm" data-action="cart-minus" data-id-produk="${escapeHtml(item.idProduk)}">−</button>
        <strong>${item.qty}</strong>
        <button class="btn btn-secondary btn-sm" data-action="cart-plus" data-id-produk="${escapeHtml(item.idProduk)}">+</button>
      </div>
      <div class="li-right">
        <div class="li-value">${formatRupiah(item.hargaSatuan * item.qty)}</div>
      </div>
    </div>
  `).join('');
}
...
registerScreen(
  ...
  async function renderKasir() {
    ...
    root.innerHTML = `
      <div class="container">
        ...
        <div class="search-bar">
          <span>🔎</span>
          <input type="search" placeholder="Cari nama obat atau kode…" data-kasir-search>
        </div>
        <div data-kasir-results></div>
        ...
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;">
            <strong>Total</strong>
            <strong data-cart-total>${formatRupiah(totalKeranjang())}</strong>
          </div>
          <div style="height:10px"></div>
          <button class="btn btn-primary" data-action="checkout">
            Proses & Simpan · <span data-cart-count>${jumlahItemKeranjang()}</span>
          </button>
        </div>
      </div>
    `;
    ...
    try {
      products = await muatProdukKasir();
    } catch (error) {
      results.innerHTML = `
        <div class="card">
          <div class="empty-state" style="padding:20px 10px;">
            Tidak dapat memuat obat.<br>
    ...
    function renderResults(keyword = '') {
      ...
      if (!filtered.length) {
        results.innerHTML = '<div class="empty-state">Obat tidak ditemukan.</div>';
        return;
      }

      results.innerHTML = filtered.map(p => `
        <button class="list-item" style="width:100%;border:none;text-align:left;" data-action="add-cart" data-id-produk="${escapeHtml(p.idProduk)}">
          <div class="li-main">
            <div class="li-title">${escapeHtml(p.nama)}</div>
            <div class="li-sub">${escapeHtml(p.kode)} · ${formatRupiah(p.hargaJual)}</div>
          </div>
          <div class="li-right">
            <div class="li-value">Stok ${p.stok}</div>
          </div>
        </button>
      `).join('');
    }
...
async function checkoutKeranjang() {
  if (!AppState.user) {
    toast('Sesi tidak tersedia. Silakan login lagi.', 'error');
    return;
  }
  ...
  const data = withIdUser({
    ...
    total: total,
    idPelanggan: AppState.cartCustomer?.idPelanggan || null,
...
function bindGlobalEvents() {
  ...
  document.addEventListener('click', event => {
    ...
    if (action === 'cart-minus') {
      ubahQtyKeranjang(target.dataset.idProduk, -1);
      return;
    }
    ...
    if (action === 'add-cart') {
      const product = AppState.produkCache.find(
        p => String(p.idProduk) === String(target.dataset.idProduk)
      );

      if (product) {
        tambahKeKeranjang(product, 1);
      }
    ...
    if (action === 'checkout') {
      checkoutKeranjang().catch(tampilkanError);
      return;
    }
...
async function initApp() {
  ...
  if (splash) {
    setTimeout(() => splash.classList.add('fade-out'), 250);
  }
...
document.addEventListener('DOMContentLoaded', () => {
  initApp().catch(error => {
    ...
    toast('Aplikasi gagal diinisialisasi: ' + errorMessage(error), 'error');
  });
});

// Public API yang memang diperlukan features-runtime.js.
